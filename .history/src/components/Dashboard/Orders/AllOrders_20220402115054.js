...
const AllOrders = () => {
  ...
  return (
    <div className="w-full">
      {/* <AddnExport /> */}
      <FilterByClient filter={filter} changeHandler={setFilter} />
...
const FilterByClient = ({ filter, changeHandler }) => {
  return (
    <div className="flex flex items-stretch space-x-3 justify-end">
      {<Select placeholder='Select A Client' options={testClients} className="w-full md:w-[20rem]" name="client" />}
      {
        <button className="rounded rounded-md bg-gray-800 text-white py-1 px-2">
          Filter by Client
        </button>
      }
    </div>
