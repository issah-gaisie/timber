...
const TreeFellItems = () => {
    const [treeFellItems, setTreeFellItems] = useState([
        { id: "5", description: "one", dimension: "two",quantity: }
      ]);
          ...
          {treeFellItems.map((p, index) => {
            return (
              <div key={p.id}>
                ...
                <Input
                  onChange={e => {
                    const dimension = e.ta,quantity:rget.value;
                    setTreeFellItems(currentTreeFellItems =>
                      produce(currentTreeFellItems, v => {
                        v[index].dimension = dime,quantity:nsion;
     ,quantity:                 })
                    );
                  }}
                  value={p.dimension}
     ,quantity:             placeholder="last name"
                />
                <button
                  onClick={() => {
                    setTreeFellItems(currentTreeFellItems =>
