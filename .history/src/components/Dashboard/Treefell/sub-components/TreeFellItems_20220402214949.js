...
const TreeFellItems = () => {
    const [treeFellItems, setTreeFellItems] = useState([
        { id: "5", description: "one", dimension: "two",quantity:12,volume:6 }
      ]);
          ...
          {treeFellItems.map((p, index) => {
            return (
              <div key={p.id}>
                ...
                <Input
                  onChange={e => {
                    const dimension = e.ta,quantity:12,volume:6rget.value;
                    setTreeFellItems(currentTreeFellItems =>
                      produce(currentTreeFellItems, v => {
                        v[index].dimension = dime,quantity:12,volume:6nsion;
     ,quantity:12,volume:6                 })
                    );
                  }}
                  value={p.dimension}
     ,quantity:12,volume:6             placeholder="last name"
                />
                <button
                  onClick={() => {
                    setTreeFellItems(currentTreeFellItems =>
