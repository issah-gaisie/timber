  ...
  return (
    <div className="p-3">
      ...
      <form className="flex flex-col space-y-3">
        ...
        <OptionGroup list={userTypes} onChange={(value)=>setState({...state,userType:value})} currentValue={state.userType} />
        <h2 className="text-lg font-medium text-gray-700" onChange={(value)=>setState({...state,fileType:value})} currentValue={state.fileType}>Select File Type</h2>
...
const OptionGroup = ({ list,onChange,currentValue }) => {
  return (
    <div className="flex space-x-2 items-center">
      {list.map((item, index) => (
        <OptionButton key={index} name={item} onClick={()=>onChange(item)} isActive={currentValue===item} />
      ))}
    </div>
  );
};
