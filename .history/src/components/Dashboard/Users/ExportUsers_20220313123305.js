...
const OptionButton = ({ name, onClick,isActive }) => {
  return (
    <button
      className={`px-3 border border-lime-700 uppercase py-1.5 rounded rounded-lg bg-lime-100 text-lime-700 hover:text-white hover:bg-lime-700 ${isActive?'bg-lime-700 ':''}`}
      onClick={onClick}
    >
      {name}
    </button>
