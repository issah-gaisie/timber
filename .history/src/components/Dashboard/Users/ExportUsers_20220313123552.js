...
const OptionButton = ({ name, onClick, isActive }) => {
  return (
    <button
    type="button"
      className={`px-3 border border-green-700 uppercase py-1.5 rounded rounded-lg bg-green-100  hover:text-white hover:bg-green-700 ${
        isActive ? "bg-green-700 text-white" : "text-green-700"
      }`}
      onClick={onClick}
    >
      {name}
