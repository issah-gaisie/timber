...
const NotificationDropdown = ({ visible, hide }) => {
  ...
  return (
    <div
      hidden={!visible}
      onMouseDown={hide}
      className={`w-96 absolute top-[140%] right-0 h-30 bg-white border border-gray-300 rounded rounded-md shadow-md z-40`}
    >
...
const NotificationItem = ({
  ...
  return (
    <p className="w-full last:border-none rounded-t rounded-b-none rounded-md last:rounded-b last:rounded-md border-b border-gray-300 p-2 text-gray-700 hover:bg-gray-200">
      {item}
    </p>
  );
};
