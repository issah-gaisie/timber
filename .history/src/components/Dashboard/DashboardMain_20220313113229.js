  ...
  return (
    <div className="w-full h-full p-6 flex flex-col items-start">
      ...
      <div className="w-full grid grid-cols-1 sm:grid-cols-3 md:grid-cols-4 gap-4">
        ...
        <LabelAndValue label={'Active Users'} value={state.activeUsers} className='text-green-500/50' active />
        <LabelAndValue label={'Deactivated Users'} value={state.deactivatedUsers} className='text-red-500/50' />
      </div>
    </div>
  );
};
...
const LabelAndValue=(props)=>{
  ...
  return(
    ...
    <span className="text-lg font-medium text-gray-300 uppercase">
      ...
      <AnimatedNumber
        value={value}
        className={`text-5xl font-bold text-slate-500/50 ${className}`}
