...
const UserCard=(props)=>{
  const {name='Issah',role="Admin",profileImage=manCTimber,active=true}=props
  return(
    <div className="bg-white rounded rounded lg flex justify-center items-center flex-col">
        <div className="h-24 w-24">
          <img src={profileImage} alt="" className="object-cover rounded rounded-full h-full w-full " />
        </div>
    </div>
  );
}
