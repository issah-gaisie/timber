...
const SideBar = () => {
  return (
    <nav className="group h-full bg-white shadow-xs basis-1/12 flex-col items-center space-y-10 pt-10 hidden md:flex">
      {dashboardNavigation.map((navItem) => (
        <NavLink to={navItem.path}>
          {<navItem.Icon className="text-4xl text-gray-600 group-hover:h"  />}
          {<navItem.IconFilled className="hidden text-4xl text-gray-600 group-hover:block" />}
        </NavLink>
      ))}
    </nav>
  );
