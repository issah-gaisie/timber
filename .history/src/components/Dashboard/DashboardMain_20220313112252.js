  ...
  return (
    <div className="w-full h-full p-6 flex flex-col items-start">
      ...
      <div className="w-full grid grid-cols-1 sm:grid-cols-4 gap-4">
        ...
        <LabelAndValue label={'Active Users'} value={state.activeUsers} className='ring ring-green-500/75 bg-green-100' active />
        <LabelAndValue label={'Deactivated Users'} value={state.deactivatedUsers} className='ring ring-red-500/75 bg-red-100 ' />
...
const LabelAndValue=(props)=>{
  const {label,value,className,active}=props;
  const formatValue = (value) => value.toFixed(0);
  return(
